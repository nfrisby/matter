...
generateMatter :: QC.Gen (Matter Vector (Bytes X) Decimal Symbol (Text NonEmpty X) P)
generateMatter =
    QC.sized $ \sz ->
...
generateSequence :: QC.Gen (Matter Vector (Bytes X) Decimal Symbol (Text NonEmpty X) P)
generateSequence =
    QC.sized $ \sz -> do
...
generateSequencePart :: QC.Gen (SequencePart P (Matter Vector (Bytes X) Decimal Symbol (Text NonEmpty X) P))
...
generateAtom :: QC.Gen (Flat (Bytes X) Decimal Symbol (Text NonEmpty X) P)
generateAtom =
    pure $ Atom $ MkSymbol MkP MkP

generateDecimal :: QC.Gen (Flat (Bytes X) Decimal Symbol (Text NonEmpty X) P)
generateDecimal =
    QC.sized $ \sz ->
        (\mbSgn (fpart, epart) -> Number $ DecimalLit mbSgn MkP MkP fpart epart)
    <$> genMbSgn
...
generateText :: QC.Gen (Flat (Bytes X) Decimal Symbol (Text NonEmpty X) P)
...
generateVariant :: QC.Gen (Matter Vector (Bytes X) Decimal Symbol (Text NonEmpty X) P)
generateVariant =
    QC.sized $ \sz -> do
        n <- frequency $
            (  (30,                pure 1)            NE.:|)
        ...
        pure $ foldr ($) x $ replicate n $ Variant $ MkSymbol MkP MkP

generateParen :: QC.Gen (Matter Vector (Bytes X) Decimal Symbol (Text NonEmpty X) P)
generateParen =
    QC.sized $ \sz -> do
        x <- QC.resize (max 1 sz - 1) generateMatter
        pure $ Paren MkP x MkP

generateMetaGT :: QC.Gen (Matter Vector (Bytes X) Decimal Symbol (Text NonEmpty X) P)
generateMetaGT =
    QC.sized $ \sz -> do
...
generatePinMetaLT :: QC.Gen (Matter Vector (Bytes X) Decimal Symbol (Text NonEmpty X) P)
generatePinMetaLT =
    QC.sized $ \sz -> do
...
shrinkMatter :: Matter Vector (Bytes X) Decimal Symbol (Text nesequ X) P -> [Matter Vector (Bytes X) Decimal Symbol (Text nesequ X) P]
shrinkMatter = \case
    Flat flt -> map Flat $ flat flt
    Variant s x -> x  : [Variant s x' | x' <- shrinkMatter x]
    Sequence l xs r ->
  ...
  where
    flat = \case
        ...
        Number (DecimalLit mbSign l r fpart epart) -> map Number $ case (fpart, epart) of
            (NothingFraction, NothingExponent) -> []
            (JustFraction{}, NothingExponent) -> [DecimalLit mbSign l r NothingFraction epart]
            (NothingFraction, JustExponent{}) -> [DecimalLit mbSign l r fpart NothingExponent]
            (JustFraction{}, JustExponent{}) -> [DecimalLit mbSign l r fpart NothingExponent, DecimalLit mbSign l r NothingFraction epart]
        Text{} -> []
